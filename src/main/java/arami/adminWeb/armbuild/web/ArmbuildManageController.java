...
public class ArmbuildManageController {
	 ...
	 * Body: JSON (gubun1, gubun2, buildNm, dayVal?, buildDesc?) — BUILD_ID는 DB 채번
	 */
	@PostMapping(consumes = MediaType.APPLICATION_JSON_VALUE, produces = "application/json;charset=UTF-8")
	public ResponseEntity<ArmbuildInsertResponse> insert(@RequestBody @Valid ArmbuildInsertRequest request) {
		try {
			return ResponseEntity.ok(armbuildManageService.insertArmbuild(request));
		} catch (Exception e) {
			log.error("armbuild insert: {}", e.getMessage(), e);
			ArmbuildInsertResponse response = new ArmbuildInsertResponse();
			response.setResult("01");
			response.setMessage("건축물용도 등록 중 오류가 발생했습니다.");
			if ("true".equals(EgovProperties.getProperty("Globals.debug"))) {
				e.printStackTrace();
			}
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
